...
GLuint TexLoading::loadTexture(const std::string &fName) {
    int width, height;
    unsigned char * data = TexLoading::loadPixels(fName, width, height);
    GLuint tex = 0;
    if( data != nullptr ) {
        glGenTextures(1, &tex);
        ...
        TexLoading::deletePixels(data);
    }

    return tex;
}

GLuint TexLoading::loadCubeMap(const std::string &baseName, const std::string &extention , bool flip) {
    GLuint texID;
        ...
        std::string texName = baseName + "_" + suffixes[0] + extention;
        GLubyte * data = TexLoading::loadPixels(texName, w, h, false);
    ...
    GL_CHECK_ERRORS;
        ...
        for( int i = 1; i < 6; i++ ) {
            std::string texName = baseName + "_" + suffixes[i] + extention;
            data = TexLoading::loadPixels(texName, w, h, false);
...
GLuint TexLoading::loadHdrCubeMap(const std::string &baseName) {
    GLuint texID;
    glGenTextures(1, &texID);
    glBindTexture(GL_TEXTURE_CUBE_MAP, texID);
...
unsigned char *TexLoading::loadPixels(const std::string &fName, int &width, int &height, bool flip) {
    int bytesPerPix;
...
void TexLoading::deletePixels(unsigned char * data) {
    stbi_image_free(data);
}
